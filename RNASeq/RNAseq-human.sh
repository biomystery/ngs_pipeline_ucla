...
#                      fastqc -t $NPROC_PER_SAMPLE -outdir $WORK_DIR {} \
...
cd $WORK_DIR
WORK_DIR=$WORK_DIR'/fastqc';mkdir -p $WORK_DIR
#ls -1 *.fastq | xargs -n1 -P $SAMPLE_NO -i \
#                      fastqc -t $NPROC_PER_SAMPLE -outdir $WORK_DIR {} \
#                      1>>$LOG_FILE 2>>$LOG_ERR_FILE
...
STEP='04filter2'
WORK_DIR=$PARENT_DIR'/'$STEP; mkdir -p $WORK_DIR;


ls -1 *.bam | xargs -n1 -P $SAMPLE_NO -i \
                     filterfun.sh {} $WORK_DIR $NPROC_PER_SAMPLE \
                     | tee -a $LOG_FILE

ls *.bam |parallel --progress -j $SAMPLE_NO  filterfun.sh {} $WORK_DIR $NPRO_PER_SAMPLE | tee -a $LOG_ERR_FILE 
...
qualmapfun (){
    ...
    while read data; do
        ...
        mkdir -p $WORK_DIR'/'$nameStr
        
        if [ `echo $int1" % 2" | bc` -eq 0 ]
        then
            echo -e "caculating $int1/$SAMPLE_NO samples \n"
            int1=$((int1+int2))
            JAVA_OPTS="-Djava.awt.headless=true" qualimap rnaseq -bam "$data" -gtf /home/shared/ngs_indexes/models/hs/hg38/gencode.v23.refchrom.annotation.gtf -p strand-specific-reverse -outdir $WORK_DIR/$nameStr --java-mem-size=4G            
        else
            echo -e "caculating $int1/$SAMPLE_NO samples \n"
            int1=$((int1+int2))
            JAVA_OPTS="-Djava.awt.headless=true" qualimap rnaseq -bam "$data" -gtf /home/shared/ngs_indexes/models/hs/hg38/gencode.v23.refchrom.annotation.gtf -p strand-specific-reverse -outdir $WORK_DIR/$nameStr --java-mem-size=4G &
...
featureCounts -T $TOTAL_PROC_NO -s 2 -t exon -g gene_id -a /home/shared/ngs_indexes/models/hs/hg38/gencode.v23.refchrom.annotation.gtf -o $WORK_DIR/counts-gene.txt *.bam 1>>$LOG_ERR_FILE 2>>$LOG_FILE
...
trackfun (){
    while read data; do
        nameStr=$(echo "$data"| cut -f1 -d".")
        bam2wig.py -i "$data" -s /home/shared/ngs_indexes/genomes/mm/mm10.chrom.sizes -t 500000000 -d '+-,-+' -o $WORK_DIR/$nameStr".bw" & 
    done
}
